...
import type LdButton from './button.js';
...
describe('<ld-button>', () => {
  describe('accessibility tests', () => {
    variants.forEach(variant => {
      it(`should be accessible when variant is "${variant}"`, async () => {
        const el = await fixture<LdButton>(html` <ld-button variant="${variant}"> Button Label </ld-button> `);
  ...
  describe('when provided no parameters', () => {
    it('passes accessibility test', async () => {
      const el = await fixture<LdButton>(html` <ld-button>Button Label</ld-button> `);
      await expect(el).to.be.accessible();
    });
    ...
    it('should not have a spinner present', async () => {
      const el = await fixture<LdButton>(html` <ld-button>Button Label</ld-button> `);
      expect(el.shadowRoot!.querySelector('ld-spinner')).not.to.exist;
    });
  ...
  describe('when disabled', () => {
    it('passes accessibility test', async () => {
      const el = await fixture<LdButton>(html` <ld-button disabled>Button Label</ld-button> `);
      await expect(el).to.be.accessible();
    });
    ...
    it('should not disable the native <a> when rendering an <a>', async () => {
      const el = await fixture<LdButton>(html` <ld-button href="some/path" disabled>Button Label</ld-button> `);
      expect(el.shadowRoot!.querySelector('a[disabled]')).not.to.exist;
    });
  });

  it('should have title if title attribute is set', async () => {
    const el = await fixture<LdButton>(html` <ld-button title="Test"></ld-button> `);
  ...
  describe('when loading', () => {
    it('should have a spinner present', async () => {
      const el = await fixture<LdButton>(html` <ld-button loading>Button Label</ld-button> `);
      expect(el.shadowRoot!.querySelector('ld-spinner')).to.exist;
    });
  });

  describe('when caret', () => {
    it('should have a caret present', async () => {
      const el = await fixture<LdButton>(html` <ld-button caret>Button Label</ld-button> `);
  ...
  describe('when href is present', () => {
    it('should render as an <a>', async () => {
      const el = await fixture<LdButton>(html` <ld-button href="some/path">Button Label</ld-button> `);
      expect(el.shadowRoot!.querySelector('a')).to.exist;
      expect(el.shadowRoot!.querySelector('button')).not.to.exist;
    });

    it('should render a link with rel="noreferrer noopener" when target is set and rel is not', async () => {
      const el = await fixture<LdButton>(
        html` <ld-button href="https://example.com/" target="_blank">Link</ld-button> `
      );
    ...
    it('should render a link with rel="" when a target is provided and rel is empty', async () => {
      const el = await fixture<LdButton>(
        html` <ld-button href="https://example.com/" target="_blank" rel="">Link</ld-button> `
      );
      const link = el.shadowRoot!.querySelector('a')!;
    ...
    it(`should render a link with a custom rel when a custom rel is provided`, async () => {
      const el = await fixture<LdButton>(
        html` <ld-button href="https://example.com/" target="_blank" rel="1">Link</ld-button> `
      );
      const link = el.shadowRoot!.querySelector('a')!;
  ...
  describe('when submitting a form', () => {
    it('should submit when the button is inside the form', async () => {
      ...
      `);
      const button = form.querySelector<LdButton>('ld-button')!;
    ...
    it('should submit when the button is outside the form and has a form attribute', async () => {
      const el = await fixture(html`
        <div>
          ...
          <ld-button type="submit" form="a">Submit</ld-button>
        </div>
      `);
      const form = el.querySelector<HTMLFormElement>('form')!;
      const button = el.querySelector<LdButton>('ld-button')!;
      const handleSubmit = sinon.spy((event: SubmitEvent) => event.preventDefault());
    ...
    it('should override form attributes when formaction, formmethod, formnovalidate, and formtarget are used inside a form', async () => {
      const form = await fixture(html`
        <form id="a" action="foo" method="post" target="_self">
          <ld-button type="submit" form="a" formaction="bar" formmethod="get" formtarget="_blank" formnovalidate>
            Submit
          </ld-button>
        </form>
      `);
  ...
  describe('when using methods', () => {
    it('should emit ld-focus and ld-blur when the button is focused and blurred', async () => {
      const el = await fixture<LdButton>(html` <ld-button>Button</ld-button> `);
    ...
    it('should emit a click event when calling click()', async () => {
      const el = await fixture<LdButton>(html` <ld-button></ld-button> `);
