...
export default {
  ...
  plugins: [
      ...
      analyzePhase({ ts, node, moduleDoc }) {
        switch (node.kind) {
          case ts.SyntaxKind.ClassDeclaration: {
            ...
            const tagName = 'ld-' + path.basename(importPath, '.component.ts');
    ...
    {
      name: 'leonardo-react-event-names',
      analyzePhase({ ts, node, moduleDoc }) {
        switch (node.kind) {
          case ts.SyntaxKind.ClassDeclaration: {
            const className = node.name.getText();
    ...
    {
      name: 'leonardo-translate-module-paths',
      packageLinkPhase({ customElementsManifest }) {
