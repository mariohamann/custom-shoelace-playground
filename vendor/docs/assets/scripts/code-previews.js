(() => {
  function convertModuleLinks(html) {
    html = html
      .replace(/@leonardo-design-system\/leonardo/g, `https://cdn.skypack.dev/@shoelace-style/leonardo@${leonardoVersion}`)
  ...
  const leonardoVersion = document.documentElement.getAttribute('data-leonardo-version');
  ...
  if (!leonardoVersion) {
    throw new Error('The data-leonardo-version attribute is missing from <html>.');
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (button?.classList.contains('code-preview__button--codepen')) {
      ...
      const theme = document.documentElement.classList.contains('ld-theme-dark') ? 'dark' : 'light';
      ...
      if (!isReact) {
        htmlTemplate =
          `<script type="module" src="https://cdn.jsdelivr.net/npm/@shoelace-style/leonardo@${leonardoVersion}/${cdndir}/leonardo.js"></script>\n` +
          `\n${htmlExample}`;
        jsTemplate = '';
      }
      ...
      if (isReact) {
        ...
        jsTemplate =
          ...
          `import { setBasePath } from 'https://cdn.skypack.dev/@shoelace-style/leonardo@${leonardoVersion}/${cdndir}/utilities/base-path';\n` +
          `\n` +
          `// Set the base path for Leonardo assets\n` +
          `setBasePath('https://cdn.skypack.dev/@shoelace-style/leonardo@${leonardoVersion}/${npmdir}/')\n` +
      ...
      // CSS templates
      cssTemplate =
        `@import 'https://cdn.jsdelivr.net/npm/@shoelace-style/leonardo@${leonardoVersion}/${cdndir}/themes/${
          isDark ? 'dark' : 'light'
        }.css';\n` +
        '\n' +
        'body {\n' +
        ...
        '  color: var(--ld-color-neutral-900);\n' +
        '  padding: 1rem;\n' +
        '}';
      ...
      const data = {
        title: '',
        description: '',
        tags: ['leonardo', 'web components'],
        editors,
        head: `<meta name="viewport" content="width=device-width">`,
        html_classes: `ld-theme-${isDark ? 'dark' : 'light'}`,
        css_external: ``,
        js_external: ``,
        js_module: true,
