...
(() => {
  ...
  function setTheme(newTheme) {
    ...
    document.documentElement.classList.toggle('ld-theme-dark', isDark());
  }

  function updateSelection() {
    const menu = document.querySelector('#theme-selector ld-menu');
    if (!menu) return;
    [...menu.querySelectorAll('ld-menu-item')].map(item => (item.checked = item.getAttribute('value') === theme));
  }

  let theme = getTheme();

  // Selection is not preserved when changing page, so update when opening dropdown
  document.addEventListener('ld-show', event => {
  ...
  document.addEventListener('ld-select', event => {
    const menu = event.target.closest('#theme-selector ld-menu');
    if (!menu) return;
...
(() => {
  function updateVersion() {
    ...
    if (location.hostname === 'next.leonardo.style') el.textContent = 'Next';
