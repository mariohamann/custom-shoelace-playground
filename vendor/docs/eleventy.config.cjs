...
const leonardoFlavoredMarkdown = require('./_utilities/markdown.cjs');
...
module.exports = function (eleventyConfig) {
  ...
  // Global data
  //
  eleventyConfig.addGlobalData('baseUrl', 'https://leonardo.style/'); // the production URL
  ...
  //
  eleventyConfig.setLibrary('md', leonardoFlavoredMarkdown);

  //
  // Filters
  //
  eleventyConfig.addFilter('markdown', content => {
    return leonardoFlavoredMarkdown.render(content);
  });

  eleventyConfig.addFilter('markdownInline', content => {
    return leonardoFlavoredMarkdown.renderInline(content);
  });
  ...
  eleventyConfig.addFilter('removeLdPrefix', tagName => {
    return tagName.replace(/^ld-/, '');
  });
