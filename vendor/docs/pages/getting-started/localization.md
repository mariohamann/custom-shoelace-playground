...
Leonardo ships with a number of translations. The default is English (US), which also serves as the fallback locale. As such, you do not need to import the English translation. To see a list of all available translations in the latest version, [refer to this directory](https://github.com/leonardo-design-system/leonardo/tree/current/src/translations).

The location of translations depends on how you're consuming Leonardo.

- If you're using the CDN, [import them from the CDN](https://www.jsdelivr.com/package/npm/@shoelace-style/leonardo?path=%CDNDIR%%2Ftranslations)
- If you're using a bundler, import them from `@shoelace-style/leonardo/%NPMDIR%/translations/[lang].js`
...
import('/path/to/leonardo/dist/translations/de.js');
```
...
Leonardo uses English as a fallback to provide a better experience than rendering nothing or throwing an error.

### Submitting New Translations or Improvements

To contribute new translations or improvements to existing translations, please submit a pull request on GitHub. Translations are located in [`src/translations`](https://github.com/leonardo-design-system/leonardo/blob/next/src/translations) and can be edited directly on GitHub if you don't want to clone the repo locally.
...
If you have any questions, please start a [discussion](https://github.com/leonardo-design-system/leonardo/discussions) or ask in the [community chat](https://discord.gg/mg8f26C).

:::tip
Leonardo provides a localization mechanism for component internals. This is not designed to be used as localization tool for your entire application. You should use a more appropriate tool such as [i18next](https://www.i18next.com/) if you need to localize content in your app.
:::
...
<html lang="es">
  ...
  <body>
    <ld-button><!-- Spanish --></ld-button>
    <ld-button lang="ru"><!-- Russian --></ld-button>
  </body>
</html>
```
...
<html lang="es">
  ...
  <body>
    <div lang="ru">
      <ld-button><!-- still in Spanish --></ld-button>
    </div>
  </body>
</html>
```
...
```js
import { registerTranslation } from '@shoelace-style/leonardo/dist/utilities/localize';
import type { Translation } from '@shoelace-style/leonardo/dist/utilities/localize';
