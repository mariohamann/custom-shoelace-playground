...
Leonardo components are just regular HTML elements, or [custom elements](https://developer.mozilla.org/en-US/docs/Web/Web_Components/Using_custom_elements) to be precise. You can use them like any other element. Each component has detailed documentation that describes its full API, including properties, events, methods, and more.
...
```html
<ld-button size="small">Click me</ld-button>
```

Some properties are boolean, so they only have true/false values. To activate a boolean property, add the corresponding attribute without a value.

```html
<ld-button disabled>Click me</ld-button>
```
...
<script>
  const colorPicker = document.querySelector('ld-color-picker');
...
As a result, you should almost always listen for custom events instead. For example, instead of listening to `click` to determine when an `<ld-checkbox>` gets toggled, listen to `ld-change`.

```html
<ld-checkbox>Check me</ld-checkbox>

<script>
  const checkbox = document.querySelector('ld-checkbox');
  checkbox.addEventListener('ld-change', event => {
    console.log(event.target.checked ? 'checked' : 'not checked');
  });
</script>
```

All custom events are prefixed with `ld-` to prevent collisions with standard events and other libraries. Refer to a component's documentation for a complete list of its custom events.

## Methods

Some components have methods you can call to trigger various behaviors. For example, you can set focus on a Leonardo input using the `focus()` method.

```html
<ld-input></ld-input>

<script>
  const input = document.querySelector('ld-input');
  input.focus();
</script>
```
...
You might expect similarly named elements to share the same API as native HTML elements, but this is not always the case. Leonardo components **are not** designed to be one-to-one replacements for their HTML counterparts. While they usually share the same API, there may be subtle differences.

For example, `<button>` and `<ld-button>` both have a `type` attribute, but the native one defaults to `submit` while the Leonardo one defaults to `button` since this is a better default for most users.
...
<script type="module">
  await Promise.allSettled([
    ...
    customElements.whenDefined('ld-card'),
    customElements.whenDefined('ld-rating')
  ]);
...
Leonardo components are built with [Lit](https://lit.dev/), a tiny library that makes authoring custom elements easier, more maintainable, and a lot of fun! As a Leonardo user, here is some helpful information about rendering and updating you should probably be aware of.
...
Leonardo ships with a file called `vscode.html-custom-data.json` that can be used to describe it's custom elements to Visual Studio Code. This enables code completion for Leonardo components (also known as "code hinting" or "IntelliSense"). To enable it, you need to tell VS Code where the file is.

1. [Install Leonardo locally](/getting-started/installation#local-installation)
...
```js
{
  "html.customData": ["./node_modules/@shoelace-style/leonardo/dist/vscode.html-custom-data.json"]
}
```
...
Most popular editors support custom code completion with a bit of configuration. Please [submit a feature request](https://github.com/leonardo-design-system/leonardo/issues/new/choose) for your editor of choice. PRs are also welcome!
