export default function (plop) {
  plop.setHelper('tagWithoutPrefix', tag => tag.replace(/^ld-/, ''));
    ...
    prompts: [
      {
        type: 'input',
        name: 'tag',
        message: 'Tag name? (e.g. ld-button)',
        validate: value => {
          // Start with ld- and include only a-z + dashes
          if (!/^ld-[a-z-+]+/.test(value)) {
            return false;
          }
