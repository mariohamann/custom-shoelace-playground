...
import type LdAvatar from './avatar.js';
...
describe('<ld-avatar>', () => {
  ...
  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<LdAvatar>(html` <ld-avatar label="Avatar"></ld-avatar> `);
    });
  ...
  describe('when provided an image and label parameter', () => {
    ...
    before(async () => {
      el = await fixture<LdAvatar>(html`<ld-avatar image="${image}" label="${label}"></ld-avatar>`);
    });

    it('should pass accessibility tests', async () => {
      /**
       * The image element itself is ancillary, because it's parent container contains the
       * aria-label which dictates what "ld-avatar" is. This also implies that label text will
  ...
  describe('when provided initials parameter', () => {
    const initials = 'SL';
    before(async () => {
      el = await fixture<LdAvatar>(html`<ld-avatar initials="${initials}" label="Avatar"></ld-avatar>`);
    });
  ...
  describe('when image is present, the initials or icon part should not render', () => {
    ...
    const label = 'Small transparent square';
    before(async () => {
      el = await fixture<LdAvatar>(
        html`<ld-avatar image="${image}" label="${label}" initials="${initials}"></ld-avatar>`
      );
    });
  ...
  ['square', 'rounded', 'circle'].forEach(shape => {
    describe(`when passed a shape attribute ${shape}`, () => {
      before(async () => {
        el = await fixture<LdAvatar>(html`<ld-avatar shape="${shape}" label="Shaped avatar"></ld-avatar>`);
      });
  ...
  describe('when passed a <span>, on slot "icon"', () => {
    before(async () => {
      el = await fixture<LdAvatar>(html`<ld-avatar label="Avatar"><span slot="icon">random content</span></ld-avatar>`);
    });
  ...
  it('should not render the image when the image fails to load', async () => {
    el = await fixture<LdAvatar>(html`<ld-avatar></ld-avatar>`);
    el.image = 'bad_image';
