...
import type LdBadge from './badge.js';
...
describe('<ld-badge>', () => {
  ...
  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<LdBadge>(html` <ld-badge>Badge</ld-badge> `);
    });
  ...
  describe('when provided a pill parameter', () => {
    before(async () => {
      el = await fixture<LdBadge>(html` <ld-badge pill>Badge</ld-badge> `);
    });
  ...
  describe('when provided a pulse parameter', () => {
    before(async () => {
      el = await fixture<LdBadge>(html` <ld-badge pulse>Badge</ld-badge> `);
    });
  ...
  ['primary', 'success', 'neutral', 'warning', 'danger'].forEach(variant => {
    describe(`when passed a variant attribute ${variant}`, () => {
      before(async () => {
        el = await fixture<LdBadge>(html`<ld-badge variant="${variant}">Badge</ld-badge>`);
      });
