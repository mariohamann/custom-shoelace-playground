...
import type LdBreadcrumbItem from '../breadcrumb-item/breadcrumb-item.js';

/**
 * @summary Breadcrumbs provide a group of links so users can easily navigate a website's hierarchy.
 * @documentation https://leonardo.style/components/breadcrumb
 * @status stable
 * @since 2.0
 *
 * @slot - One or more breadcrumb items to display.
 * @slot separator - The separator to use between breadcrumb items. Works best with `<ld-icon>`.
 *
 * @dependency ld-icon
 *
 * @csspart base - The component's base wrapper.
 */
export default class LdBreadcrumb extends LeonardoElement {
  ...
  private handleSlotChange() {
    const items = [...this.defaultSlot.assignedElements({ flatten: true })].filter(
      item => item.tagName.toLowerCase() === 'ld-breadcrumb-item'
    ) as LdBreadcrumbItem[];
  ...
  render() {
    ...
    return html`
      ...
      <span hidden aria-hidden="true">
        <slot name="separator">
          <ld-icon name=${this.localize.dir() === 'rtl' ? 'chevron-left' : 'chevron-right'} library="system"></ld-icon>
        </slot>
      </span>
    `;
  }
