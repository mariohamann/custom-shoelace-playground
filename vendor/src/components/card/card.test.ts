...
import type LdCard from './card.js';
...
describe('<ld-card>', () => {
  ...
  describe('when provided no parameters', () => {
    before(async () => {
      el = await fixture<LdCard>(
        html` <ld-card>This is just a basic card. No image, no header, and no footer. Just your content.</ld-card> `
      );
    });
