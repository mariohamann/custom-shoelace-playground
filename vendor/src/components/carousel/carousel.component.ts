...
import LdCarouselItem from '../carousel-item/carousel-item.component.js';
 ...
 * @dependency ld-icon
 *
 * @event {{ index: number, slide: LdCarouselItem }} ld-slide-change - Emitted when the active slide changes.
 *
 * @slot - The carousel's main content, one or more `<ld-carousel-item>` elements.
 * @slot next-icon - Optional next icon to use instead of the default. Works best with `<ld-icon>`.
 * @slot previous-icon - Optional previous icon to use instead of the default. Works best with `<ld-icon>`.
 *
 ...
 */
export default class LdCarousel extends LeonardoElement {
  static styles: CSSResultGroup = styles;
  static dependencies = { 'ld-icon': LdIcon };
  ...
  private readonly slides = this.getElementsByTagName('ld-carousel-item');
  ...
  private handleScrollEnd() {
    ...
    if (firstIntersecting) {
      this.activeSlide = slides.indexOf(firstIntersecting.target as LdCarouselItem);
    }
  }
  ...
  private handleSlotChange = (mutations: MutationRecord[]) => {
    const needsInitialization = mutations.some(mutation =>
      [...mutation.addedNodes, ...mutation.removedNodes].some(
        node => LdCarouselItem.isCarouselItem(node) && !(node as HTMLElement).hasAttribute('data-clone')
      )
    );
              ...
              <div part="navigation" class="carousel__navigation">
                ...
                >
                  <slot name="previous-icon">
                    <ld-icon library="system" name="${isLtr ? 'chevron-left' : 'chevron-right'}"></ld-icon>
                  </slot>
                </button>
                ...
                >
                  <slot name="next-icon">
                    <ld-icon library="system" name="${isLtr ? 'chevron-right' : 'chevron-left'}"></ld-icon>
                  </slot>
                </button>
              </div>
            `
