...
import LdButton from '../button/button.component.js';
import LdButtonGroup from '../button-group/button-group.component.js';
import LdDropdown from '../dropdown/dropdown.component.js';
import LdIcon from '../icon/icon.component.js';
import LdInput from '../input/input.component.js';
import LdVisuallyHidden from '../visually-hidden/visually-hidden.component.js';
...
import type LdInputEvent from '../../events/ld-input.js';
 ...
 * @documentation https://leonardo.style/components/color-picker
 * @status stable
 * @since 2.0
 *
 * @dependency ld-button
 * @dependency ld-button-group
 * @dependency ld-dropdown
 * @dependency ld-input
 * @dependency ld-visually-hidden
 *
 * @slot label - The color picker's form label. Alternatively, you can use the `label` attribute.
 *
 * @event ld-blur - Emitted when the color picker loses focus.
 * @event ld-change - Emitted when the color picker's value changes.
 * @event ld-focus - Emitted when the color picker receives focus.
 * @event ld-input - Emitted when the color picker receives input.
 ...
 */
export default class LdColorPicker extends LeonardoElement implements LeonardoFormControl {
  static styles: CSSResultGroup = styles;

  static dependencies = {
    'ld-button-group': LdButtonGroup,
    'ld-button': LdButton,
    'ld-dropdown': LdDropdown,
    'ld-icon': LdIcon,
    'ld-input': LdInput,
  ...
  @query('[part~="input"]') input: LdInput;
  @query('.color-dropdown') dropdown: LdDropdown;
  ...
  private handleInputChange(event: LdChangeEvent) {
    const target = event.target as HTMLInputElement;
    const oldValue = this.value;

    // Prevent the <ld-input>'s ld-change event from bubbling up
  ...
  private handleInputInput(event: LdInputEvent) {
    this.formControlController.updateValidity();

    // Prevent the <ld-input>'s ld-input event from bubbling up
    event.stopPropagation();
  }
  ...
  reportValidity() {
    if (!this.inline && !this.validity.valid) {
      ...
      this.dropdown.show();
      this.addEventListener('ld-after-show', () => this.input.reportValidity(), { once: true });

      if (!this.disabled) {
        // By standards we have to emit a `ld-invalid` event here synchronously.
  ...
  render() {
      ...
      >
        ...
        <div class="color-picker__user-input" aria-live="polite">
          <ld-input
            ...
            @ld-change=${this.handleInputChange}
            @ld-input=${this.handleInputInput}
            @ld-invalid=${this.handleInputInvalid}
            @ld-blur=${this.stopNestedEventPropagation}
            @ld-focus=${this.stopNestedEventPropagation}
          ></ld-input>
    ...
    return html`
      <ld-dropdown
        ...
        ?disabled=${this.disabled}
        ?hoist=${this.hoist}
        @ld-after-hide=${this.handleAfterHide}
      >
        <button
          part="trigger"
          slot="trigger"
