...
import type LdColorPicker from './color-picker.js';

describe('<ld-color-picker>', () => {
  describe('when the value changes', () => {
    it('should not emit ld-change or ld-input when the value is changed programmatically', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker></ld-color-picker> `);
      const color = 'rgb(255, 204, 0)';

      el.addEventListener('ld-change', () => expect.fail('ld-change should not be emitted'));
      el.addEventListener('ld-input', () => expect.fail('ld-change should not be emitted'));
      el.value = color;
      await el.updateComplete;
    });

    it('should emit ld-change and ld-input when the color grid selector is moved', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker></ld-color-picker> `);
    ...
    it('should emit ld-change and ld-input when the hue slider is moved', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker></ld-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit ld-change and ld-input when the opacity slider is moved', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker opacity></ld-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit ld-change and ld-input when toggling the format', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker value="#fff"></ld-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should render the correct swatches when passing a string of color values', async () => {
      const el = await fixture<LdColorPicker>(
        html` <ld-color-picker swatches="red; #008000; rgb(0,0,255);"></ld-color-picker> `
      );
    ...
    it('should emit ld-change and ld-input when clicking on a swatch', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker swatches="red; green; blue;"></ld-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit ld-change and ld-input when selecting a color with the keyboard', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker></ld-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit ld-change and ld-input when selecting hue with the keyboard', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker></ld-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit ld-change and ld-input when selecting opacity with the keyboard', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker opacity></ld-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit ld-change and ld-input when entering a value in the color input and pressing enter', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker opacity></ld-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    ...
    it('should emit ld-change and ld-input when entering a value in the color input and blurring the field', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker opacity></ld-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
      const input = el.shadowRoot!.querySelector<HTMLElement>('[part~="input"]')!;
      const changeHandler = sinon.spy();
    ...
    it('should render the correct format when selecting a swatch of a different format', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker format="rgb"></ld-color-picker> `);
      const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
      const changeHandler = sinon.spy();
      const inputHandler = sinon.spy();
  ...
  it('should render in a dropdown', async () => {
    const el = await fixture<LdColorPicker>(html` <ld-color-picker></ld-color-picker> `);
    const dropdown = el.shadowRoot!.querySelector('ld-dropdown');

    expect(dropdown).to.exist;
  });

  it('should not render in a dropdown when inline is enabled', async () => {
    const el = await fixture<LdColorPicker>(html` <ld-color-picker inline></ld-color-picker> `);
    const dropdown = el.shadowRoot!.querySelector('ld-dropdown');
  ...
  it('should display a color when an initial value is provided', async () => {
    const el = await fixture<LdColorPicker>(html` <ld-color-picker value="#000"></ld-color-picker> `);
    const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]');
  ...
  it('should display a color with opacity when an initial value with opacity is provided', async () => {
    const el = await fixture<LdColorPicker>(html` <ld-color-picker opacity value="#ff000050"></ld-color-picker> `);
    const trigger = el.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
  ...
  it('should emit ld-focus when rendered as a dropdown and focused', async () => {
    const el = await fixture<LdColorPicker>(html`
      <div>
        <ld-color-picker></ld-color-picker>
        <button type="button">Click me</button>
      </div>
    `);
    const colorPicker = el.querySelector('ld-color-picker')!;
    const trigger = colorPicker.shadowRoot!.querySelector<HTMLButtonElement>('[part~="trigger"]')!;
    const button = el.querySelector('button')!;
    const focusHandler = sinon.spy();
    const blurHandler = sinon.spy();

    colorPicker.addEventListener('ld-focus', focusHandler);
    colorPicker.addEventListener('ld-blur', blurHandler);
  ...
  it('should emit ld-focus when rendered inline and focused', async () => {
    const el = await fixture<LdColorPicker>(html`
      <div>
        <ld-color-picker inline></ld-color-picker>
        <button type="button">Click me</button>
      </div>
    `);
    const colorPicker = el.querySelector('ld-color-picker')!;
  ...
  it('should focus and blur when calling focus() and blur() and rendered as a dropdown', async () => {
    const colorPicker = await fixture<LdColorPicker>(html` <ld-color-picker></ld-color-picker> `);
    const focusHandler = sinon.spy();
    const blurHandler = sinon.spy();
  ...
  it('should focus and blur when calling focus() and blur() and rendered inline', async () => {
    const colorPicker = await fixture<LdColorPicker>(html` <ld-color-picker inline></ld-color-picker> `);
    const focusHandler = sinon.spy();
    const blurHandler = sinon.spy();
  ...
  describe('when submitting a form', () => {
    it('should serialize its name and value with FormData', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <ld-color-picker name="a" value="#ffcc00"></ld-color-picker>
        </form>
      `);
    ...
    it('should be present in form data when using the form attribute and located outside of a <form>', async () => {
      const el = await fixture<HTMLFormElement>(html`
        <div>
          ...
          <ld-color-picker form="f" name="a" value="#ffcc00"></ld-color-picker>
        </div>
      `);
  ...
  describe('when resetting a form', () => {
    it('should reset the element to its initial value', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <ld-color-picker name="a" value="#ffffff"></ld-color-picker>
          <ld-button type="reset">Reset</ld-button>
        </form>
      `);
      const button = form.querySelector('ld-button')!;
      const colorPicker = form.querySelector('ld-color-picker')!;
  ...
  describe('when using constraint validation', () => {
    ...
    it('should be invalid when required and empty', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker required></ld-color-picker> `);
      expect(el.checkValidity()).to.be.false;
    });

    it('should be invalid when required and disabled is removed', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker disabled required></ld-color-picker> `);
      el.disabled = false;
      await el.updateComplete;
      expect(el.checkValidity()).to.be.false;
    });
    ...
    it('should receive the correct validation attributes ("states") when valid', async () => {
      const el = await fixture<LdColorPicker>(html` <ld-color-picker required value="#fff"></ld-color-picker> `);
  ...
  runFormControlBaseTests('ld-color-picker');
});
