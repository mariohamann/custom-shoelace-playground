...
import type LdFormatBytes from './format-bytes';

describe('<ld-format-bytes>', () => {
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<LdFormatBytes>(html` <ld-format-bytes></ld-format-bytes> `);
  ...
  describe('bits', () => {
    ...
    results.forEach(expected => {
      it('bits : display formats', async () => {
        const el = await fixture<LdFormatBytes>(html` <ld-format-bytes unit="bit"></ld-format-bytes> `);
        // short
        el.value = expected.value;
        await elementUpdated(el);
        expect(el.shadowRoot?.textContent).to.equal(expected.short);
