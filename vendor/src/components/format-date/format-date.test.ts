...
import type LdFormatDate from './format-date';

describe('<ld-format-date>', () => {
  describe('defaults ', () => {
    ...
    it('default properties', async () => {
      const el = await fixture<LdFormatDate>(html` <ld-format-date></ld-format-date> `);
  ...
  describe('lang property', () => {
    ...
    results.forEach(setup => {
      it(`date has correct language format: ${setup.lang}`, async () => {
        const el = await fixture<LdFormatDate>(
          html`
            <ld-format-date .date="${new Date(new Date().getFullYear(), 0, 1)}" lang="${setup.lang}"></ld-format-date>
          `
        );
  ...
  describe('era property', () => {
    ...
    eras.forEach((eraFormat: 'narrow' | 'short' | 'long') => {
      it(`date has correct era format: ${eraFormat}`, async () => {
        const el = await fixture<LdFormatDate>(
          html`
            <ld-format-date .date="${new Date(new Date().getFullYear(), 0, 1)}" era="${eraFormat}"></ld-format-date>
          `
        );
  ...
  describe('year property', () => {
    ...
    yearFormats.forEach((yearFormat: 'numeric' | '2-digit') => {
      it(`date has correct year format: ${yearFormat}`, async () => {
        const el = await fixture<LdFormatDate>(
          html`
            <ld-format-date .date="${new Date(new Date().getFullYear(), 0, 1)}" year="${yearFormat}"></ld-format-date>
          `
        );
  ...
  describe('month property', () => {
    ...
    monthFormats.forEach((monthFormat: 'numeric' | '2-digit' | 'narrow' | 'short' | 'long') => {
      it(`date has correct month format: ${monthFormat}`, async () => {
        const el = await fixture<LdFormatDate>(
          html`
            <ld-format-date .date="${new Date(new Date().getFullYear(), 0, 1)}" month="${monthFormat}"></ld-format-date>
          `
        );
  ...
  describe('day property', () => {
    ...
    dayFormats.forEach((dayFormat: 'numeric' | '2-digit') => {
      it(`date has correct day format: ${dayFormat}`, async () => {
        const el = await fixture<LdFormatDate>(
          html`
            <ld-format-date .date="${new Date(new Date().getFullYear(), 0, 1)}" day="${dayFormat}"></ld-format-date>
          `
        );
  ...
  describe('hour property', () => {
    ...
    hourFormats.forEach((hourFormat: 'numeric' | '2-digit') => {
      it(`date has correct hour format: ${hourFormat}`, async () => {
        const el = await fixture<LdFormatDate>(
          html`
            <ld-format-date .date="${new Date(new Date().getFullYear(), 0, 1)}" hour="${hourFormat}"></ld-format-date>
          `
        );
