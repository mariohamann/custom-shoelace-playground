...
import type LdIconButton from './icon-button';

type LinkTarget = '_self' | '_blank' | '_parent' | '_top';

describe('<ld-icon-button>', () => {
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<LdIconButton>(html` <ld-icon-button></ld-icon-button> `);
  ...
  describe('when styling the host element', () => {
    it('renders the correct color and font size', async () => {
      const el = await fixture<LdIconButton>(
        html`
          <ld-icon-button
            library="system"
            name="check"
      ...
      const icon = el.shadowRoot!.querySelector('ld-icon')!;
  ...
  describe('when icon attributes are present', () => {
    it('renders an ld-icon from a library', async () => {
      const el = await fixture<LdIconButton>(html` <ld-icon-button library="system" name="check"></ld-icon-button> `);
      expect(el.shadowRoot?.querySelector('ld-icon')).to.exist;
    });

    it('renders an ld-icon from a src', async () => {
      const fakeId = 'test-src';
      const el = await fixture<LdIconButton>(html` <ld-icon-button></ld-icon-button> `);

      el.src = `data:image/svg+xml,${encodeURIComponent(`<svg id="${fakeId}"></svg>`)}`;

      const internalLdIcon = el.shadowRoot?.querySelector('ld-icon');

      await waitUntil(() => internalLdIcon?.shadowRoot?.querySelector('svg'), 'SVG not rendered');

      expect(internalLdIcon).to.exist;
      expect(internalLdIcon?.shadowRoot?.querySelector('svg')).to.exist;
      expect(internalLdIcon?.shadowRoot?.querySelector('svg')?.getAttribute('id')).to.equal(fakeId);
    });
  });

  describe('when href is present', () => {
    it('renders as an anchor', async () => {
      const el = await fixture<LdIconButton>(html` <ld-icon-button href="some/path"></ld-icon-button> `);
    ...
    describe('and target is present', () => {
      ['_blank', '_parent', '_self', '_top'].forEach((target: LinkTarget) => {
        it(`the anchor target is the provided target: ${target}`, async () => {
          const el = await fixture<LdIconButton>(
            html` <ld-icon-button href="some/path" target="${target}"></ld-icon-button> `
          );
    ...
    describe('and download is present', () => {
      it(`the anchor download attribute is the provided download`, async () => {
        ...
        const el = await fixture<LdIconButton>(
          html` <ld-icon-button href="some/path" download="${fakeDownload}"></ld-icon-button> `
        );
  ...
  describe('when label is present', () => {
    it('the internal aria-label attribute is set to the provided label when rendering a button', async () => {
      ...
      const el = await fixture<LdIconButton>(html` <ld-icon-button label="${fakeLabel}"></ld-icon-button> `);
    ...
    it('the internal aria-label attribute is set to the provided label when rendering an anchor', async () => {
      const fakeLabel = 'some label';
      const el = await fixture<LdIconButton>(
        html` <ld-icon-button href="some/path" label="${fakeLabel}"></ld-icon-button> `
      );
  ...
  describe('when disabled is present', () => {
    it('the internal button has a disabled attribute when rendering a button', async () => {
      const el = await fixture<LdIconButton>(html` <ld-icon-button disabled></ld-icon-button> `);
      expect(el.shadowRoot?.querySelector(`button[disabled]`)).to.exist;
    });

    it('the internal anchor has an aria-disabled attribute when rendering an anchor', async () => {
      const el = await fixture<LdIconButton>(html` <ld-icon-button href="some/path" disabled></ld-icon-button> `);
