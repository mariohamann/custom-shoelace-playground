...
import { registerIconLibrary } from '../../../dist/leonardo.js';
import type LdErrorEvent from '../../events/ld-error';
import type LdIcon from './icon';
import type LdLoadEvent from '../../events/ld-load';
...
describe('<ld-icon>', () => {
  ...
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<LdIcon>(html` <ld-icon></ld-icon> `);
    ...
    it('renders pre-loaded system icons and emits ld-load event', async () => {
      const el = await fixture<LdIcon>(html` <ld-icon library="system"></ld-icon> `);
      const listener = oneEvent(el, 'ld-load') as Promise<LdLoadEvent>;
    ...
    it('the icon is accessible', async () => {
      const el = await fixture<LdIcon>(html` <ld-icon library="system" name="check"></ld-icon> `);
  ...
  describe('when a label is provided', () => {
    it('the icon has the correct default aria attributes', async () => {
      ...
      const el = await fixture<LdIcon>(html` <ld-icon label="${fakeLabel}" library="system" name="check"></ld-icon> `);
  ...
  describe('when a valid src is provided', () => {
    it('the svg is rendered', async () => {
      ...
      const listener = oneEvent(el, 'ld-load');
  ...
  describe('new library', () => {
    it('renders icons from the new library and emits ld-load event', async () => {
      const el = await fixture<LdIcon>(html` <ld-icon library="test-library"></ld-icon> `);
      const listener = oneEvent(el, 'ld-load') as Promise<LdLoadEvent>;
    ...
    it('runs mutator from new library', async () => {
      const el = await fixture<LdIcon>(html` <ld-icon library="test-library" name="test-icon1"></ld-icon> `);
      await elementUpdated(el);
  ...
  describe('negative cases', () => {
    ...
    it("svg not rendered with an icon that doesn't exist in the library", async () => {
      const el = await fixture<LdIcon>(html` <ld-icon library="test-library" name="does-not-exist"></ld-icon> `);

      expect(el.shadowRoot?.querySelector('svg')).to.be.null;
    });

    it('emits ld-error when the file cant be retrieved', async () => {
      const el = await fixture<LdIcon>(html` <ld-icon library="test-library"></ld-icon> `);
      const listener = oneEvent(el, 'ld-error') as Promise<LdErrorEvent>;
    ...
    it("emits ld-error when there isn't an svg element in the registered icon", async () => {
      const el = await fixture<LdIcon>(html` <ld-icon library="test-library"></ld-icon> `);
      const listener = oneEvent(el, 'ld-error') as Promise<LdErrorEvent>;
  ...
  describe('svg sprite sheets', () => {
    ...
    it('Should properly grab an SVG and render it from bootstrap icons', async () => {
      ...
      const el = await fixture<LdIcon>(html`<ld-icon name="arrow-left" library="sprite"></ld-icon>`);
    ...
    it('Should render nothing if the sprite hash is wrong', async () => {
      ...
      const el = await fixture<LdIcon>(html`<ld-icon name="non-existent" library="sprite"></ld-icon>`);
    ...
    it.skip("Should produce an error if the icon doesn't exist.", async () => {
      ...
      const el = await fixture<LdIcon>(html`<ld-icon name="bad-icon" library="sprite"></ld-icon>`);
      const listener = oneEvent(el, 'ld-error') as Promise<LdErrorEvent>;
