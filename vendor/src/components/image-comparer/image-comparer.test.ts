...
import type LdImageComparer from './image-comparer';

describe('<ld-image-comparer>', () => {
  it('should render a basic before/after', async () => {
    const el = await fixture<LdImageComparer>(html`
      <ld-image-comparer>
        <div slot="before"></div>
        <div slot="after"></div>
      </ld-image-comparer>
  ...
  it('should emit change event when position changed manually', async () => {
    ...
    el.addEventListener('ld-change', handler, { once: true });
