 ...
 * @documentation https://leonardo.style/components/include
 * @status stable
 * @since 2.0
 *
 * @event ld-load - Emitted when the included file is loaded.
 * @event {{ status: number }} ld-error - Emitted when the included file fails to load due to an error.
 */
export default class LdInclude extends LeonardoElement {
  ...
  async handleSrcChange() {
    try {
      ...
      if (!file.ok) {
        this.emit('ld-error', { detail: { status: file.status } });
        return;
      }
      ...
      this.emit('ld-error', { detail: { status: -1 } });
    }
  }
