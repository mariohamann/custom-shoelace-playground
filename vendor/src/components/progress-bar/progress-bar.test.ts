...
import type LdProgressBar from './progress-bar';

describe('<ld-progress-bar>', () => {
  let el: LdProgressBar;

  describe('when provided just a value parameter', () => {
    before(async () => {
      el = await fixture<LdProgressBar>(html`<ld-progress-bar value="25"></ld-progress-bar>`);
    });
  ...
  describe('when provided a title, and value parameter', () => {
    ...
    before(async () => {
      el = await fixture<LdProgressBar>(
        html`<ld-progress-bar title="Titled Progress Ring" value="25"></ld-progress-bar>`
      );
  ...
  describe('when provided an indeterminate parameter', () => {
    ...
    before(async () => {
      el = await fixture<LdProgressBar>(
        html`<ld-progress-bar title="Titled Progress Ring" indeterminate></ld-progress-bar>`
      );
      base = el.shadowRoot!.querySelector('[part~="base"]')!;
    });
  ...
  describe('when provided a ariaLabel, and value parameter', () => {
    before(async () => {
      el = await fixture<LdProgressBar>(
        html`<ld-progress-bar ariaLabel="Labelled Progress Ring" value="25"></ld-progress-bar>`
      );
    });
    ...
    before(async () => {
      el = await fixture<LdProgressBar>(
        html`
          ...
          <ld-progress-bar ariaLabelledBy="labelledby" value="25"></ld-progress-bar>
        `
      );
    });
