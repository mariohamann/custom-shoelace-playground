...
import type LdProgressRing from './progress-ring';

describe('<ld-progress-ring>', () => {
  let el: LdProgressRing;

  describe('when provided just a value parameter', () => {
    before(async () => {
      el = await fixture<LdProgressRing>(html`<ld-progress-ring value="25"></ld-progress-ring>`);
    });
  ...
  describe('when provided a title, and value parameter', () => {
    ...
    before(async () => {
      el = await fixture<LdProgressRing>(
        html`<ld-progress-ring title="Titled Progress Ring" value="25"></ld-progress-ring>`
      );
  ...
  describe('when provided a ariaLabel, and value parameter', () => {
    before(async () => {
      el = await fixture<LdProgressRing>(
        html`<ld-progress-ring ariaLabel="Labelled Progress Ring" value="25"></ld-progress-ring>`
      );
    });
    ...
    before(async () => {
      el = await fixture<LdProgressRing>(
        html`
          ...
          <ld-progress-ring ariaLabelledBy="labelledby" value="25"></ld-progress-ring>
        `
      );
    });
