...
import type LdQrCode from './qr-code';

const getCanvas = (qrCode: LdQrCode): HTMLCanvasElement => {
...
const expectCanvasToHaveAriaLabel = (qrCode: LdQrCode, expectedLabel: string): void => {
...
const getQrCodeColors = (qrCode: LdQrCode): QrCodeColors => {
  const canvas = getCanvas(qrCode);
...
const expectQrCodeColorsToBe = (qrCode: LdQrCode, expectedColors: QrCodeColors): void => {
...
describe('<ld-qr-code>', () => {
  it('should render a component', async () => {
    const qrCode = await fixture<LdQrCode>(html` <ld-qr-code value="test data"></ld-qr-code>`);
  ...
  it('uses the label if given', async () => {
    const qrCode = await fixture<LdQrCode>(html` <ld-qr-code value="test data" label="test label"></ld-qr-code>`);

    expectCanvasToHaveAriaLabel(qrCode, 'test label');
  });

  it('sets the correct color for the qr code', async () => {
    const qrCode = await fixture<LdQrCode>(html` <ld-qr-code value="test data" fill="red"></ld-qr-code>`);

    expectQrCodeColorsToBe(qrCode, { foreground: red, background: white });
  });

  it('sets the correct background for the qr code', async () => {
    const qrCode = await fixture<LdQrCode>(
      html` <ld-qr-code value="test data" fill="red" background="blue"></ld-qr-code>`
    );
  ...
  it('has the expected size', async () => {
    const qrCode = await fixture<LdQrCode>(html` <ld-qr-code value="test data" size="100"></ld-qr-code>`);
