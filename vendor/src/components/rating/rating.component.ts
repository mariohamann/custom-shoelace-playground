 ...
 * @documentation https://leonardo.style/components/rating
 * @status stable
 * @since 2.0
 *
 * @dependency ld-icon
 *
 * @event ld-change - Emitted when the rating's value changes.
 * @event {{ phase: 'start' | 'move' | 'end', value: number }} ld-hover - Emitted when the user hovers over a value. The
 ...
 */
export default class LdRating extends LeonardoElement {
   ...
   * well with `<ld-icon>` elements.
   */
  @property() getSymbol: (value: number) => string = () => '<ld-icon name="star-fill" library="system"></ld-icon>';
