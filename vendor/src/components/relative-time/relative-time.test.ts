...
import type LdRelativeTime from './relative-time.js';

interface LdRelativeTimeTestCase {
  date: Date;
  expectedOutput: string;
}

const extractTimeElement = (relativeTime: LdRelativeTime): HTMLTimeElement | null => {
  return relativeTime.shadowRoot?.querySelector('time') || null;
};

const expectFormattedRelativeTimeToBe = async (relativeTime: LdRelativeTime, expectedOutput: string): Promise<void> => {
...
const createRelativeTimeWithDate = async (relativeDate: Date): Promise<LdRelativeTime> => {
  const relativeTime: LdRelativeTime = await fixture<LdRelativeTime>(
    html` <ld-relative-time lang="en-US"></ld-relative-time> `
  );
  relativeTime.date = relativeDate;
  return relativeTime;
};

const expectTitleToMatchLocalizedTimeString = (relativeTime: LdRelativeTime) => {
...
const testCases: LdRelativeTimeTestCase[] = [
  {
...
describe('ld-relative-time', () => {
  ...
  describe('handles time correctly', () => {
    ...
    testCases.forEach(testCase => {
      ...
      it(`shows the correct relative time given a String object: ${testCase.expectedOutput}`, async () => {
        ...
        const relativeTime: LdRelativeTime = await fixture<LdRelativeTime>(
          html` <ld-relative-time lang="en-US" date="${dateString}"></ld-relative-time> `
        );
    ...
    it('always shows numeric if requested via numeric property', async () => {
      const relativeTime: LdRelativeTime = await fixture<LdRelativeTime>(
        html` <ld-relative-time lang="en-US" numeric="always"></ld-relative-time> `
      );
    ...
    it('shows human readable form if appropriate and numeric property is auto', async () => {
      const relativeTime: LdRelativeTime = await fixture<LdRelativeTime>(
        html` <ld-relative-time lang="en-US" numeric="auto"></ld-relative-time> `
      );
      relativeTime.date = yesterday;
    ...
    it('allows to use a short form of the unit', async () => {
      ...
      const relativeTime: LdRelativeTime = await fixture<LdRelativeTime>(
        html` <ld-relative-time lang="en-US" numeric="always" format="short"></ld-relative-time> `
      );
    ...
    it('allows to use a long form of the unit', async () => {
      const twoYearsAgo = new Date(currentTime.getTime() - 2 * nonLeapYearInSeconds);
      const relativeTime: LdRelativeTime = await fixture<LdRelativeTime>(
        html` <ld-relative-time lang="en-US" numeric="always" format="long"></ld-relative-time> `
      );
      relativeTime.date = twoYearsAgo;
    ...
    it('is formatted according to the requested locale', async () => {
      const relativeTime: LdRelativeTime = await fixture<LdRelativeTime>(
        html` <ld-relative-time lang="de-DE" numeric="auto"></ld-relative-time> `
      );
      relativeTime.date = yesterday;
  ...
  it('does not display a time element on invalid time string', async () => {
    ...
    const relativeTime: LdRelativeTime = await fixture<LdRelativeTime>(
      html` <ld-relative-time lang="en-US" date="${invalidDateString}"></ld-relative-time> `
    );
