...
import type LdSplitPanel from './split-panel.js';

const DIVIDER_WIDTH_IN_PX = 4;

const getPanel = (splitPanel: LdSplitPanel, testid: string): HTMLElement => {
  const startPanel = queryByTestId<HTMLElement>(splitPanel, testid);
  expect(startPanel).not.to.be.null;
  return startPanel!;
};

const getPanelWidth = (splitPanel: LdSplitPanel, testid: string) => {
  const panel = getPanel(splitPanel, testid);
  const { width } = panel.getBoundingClientRect();
  return width;
};

const getPanelHeight = (splitPanel: LdSplitPanel, testid: string) => {
  const panel = getPanel(splitPanel, testid);
  const { height } = panel.getBoundingClientRect();
  return height;
};

const getDivider = (splitPanel: LdSplitPanel): Element => {
  const divider = splitPanel.shadowRoot?.querySelector('[part="divider"]');
  expect(divider).not.to.be.null;
  return divider!;
};

describe('<ld-split-panel>', () => {
  afterEach(async () => {
    await resetMouse();
  });

  it('should render a component', async () => {
    const splitPanel = await fixture(html` <ld-split-panel></ld-split-panel> `);

    expect(splitPanel).to.exist;
  });

  it('should be accessible', async () => {
    const splitPanel = await fixture(html`<ld-split-panel>
  ...
  describe('panel sizing horizontal', () => {
    it('has two evenly sized panels by default', async () => {
      const splitPanel = await fixture<LdSplitPanel>(html`<ld-split-panel>
    ...
    it('changes the sizing of the panels based on the position attribute', async () => {
      const splitPanel = await fixture<LdSplitPanel>(html`<ld-split-panel position="25">
        <div slot="start" data-testid="start-panel">Start</div>
        <div slot="end" data-testid="end-panel">End</div>
      </ld-split-panel>`);
    ...
    it('emits the ld-reposition	event on position change', async () => {
      const splitPanel = await fixture<LdSplitPanel>(html`<ld-split-panel>
        <div slot="start">Start</div>
        <div slot="end">End</div>
      </ld-split-panel>`);

      const repositionPromise = oneEvent(splitPanel, 'ld-reposition');
      splitPanel.position = 10;
    ...
    it('cannot be resized if disabled', async () => {
      const splitPanel = await fixture<LdSplitPanel>(html`<ld-split-panel disabled>
        <div slot="start">Start</div>
        <div slot="end">End</div>
      </ld-split-panel>`);
  ...
  describe('panel sizing vertical', () => {
    it('has two evenly sized panels by default', async () => {
      const splitPanel = await fixture<LdSplitPanel>(html`<ld-split-panel vertical style="height: 400px;">
        <div slot="start" data-testid="start-panel">Start</div>
        <div slot="end" data-testid="end-panel">End</div>
      </ld-split-panel>`);
    ...
    it('changes the sizing of the panels based on the position attribute', async () => {
      const splitPanel = await fixture<LdSplitPanel>(html`<ld-split-panel position="25" vertical style="height: 400px;">
        <div slot="start" data-testid="start-panel">Start</div>
        <div slot="end" data-testid="end-panel">End</div>
      </ld-split-panel>`);
    ...
    it('cannot be resized if disabled', async () => {
      const splitPanel = await fixture<LdSplitPanel>(html`<ld-split-panel disabled vertical style="height: 400px;">
        <div slot="start">Start</div>
        <div slot="end">End</div>
      </ld-split-panel>`);
