...
import type LdSwitch from './switch.js';

describe('<ld-switch>', () => {
  it('should pass accessibility tests', async () => {
    const el = await fixture<LdSwitch>(html` <ld-switch>Switch</ld-switch> `);
    await expect(el).to.be.accessible();
  });

  it('default properties', async () => {
    const el = await fixture<LdSwitch>(html` <ld-switch></ld-switch> `);
  ...
  it('should have title if title attribute is set', async () => {
    const el = await fixture<LdSwitch>(html` <ld-switch title="Test"></ld-switch> `);
  ...
  it('should be disabled with the disabled attribute', async () => {
    const el = await fixture<LdSwitch>(html` <ld-switch disabled></ld-switch> `);
  ...
  it('should emit ld-change when toggled with spacebar', async () => {
    const el = await fixture<LdSwitch>(html` <ld-switch></ld-switch> `);
    const changeHandler = sinon.spy();
    const inputHandler = sinon.spy();
  ...
  it('should emit ld-change and ld-input when toggled with the right arrow', async () => {
    const el = await fixture<LdSwitch>(html` <ld-switch></ld-switch> `);
    const changeHandler = sinon.spy();
    const inputHandler = sinon.spy();
  ...
  it('should emit ld-change and ld-input when toggled with the left arrow', async () => {
    const el = await fixture<LdSwitch>(html` <ld-switch checked></ld-switch> `);
    const changeHandler = sinon.spy();
    const inputHandler = sinon.spy();
  ...
  it('should not emit ld-change or ld-input when checked is set by JavaScript', async () => {
    const el = await fixture<LdSwitch>(html` <ld-switch></ld-switch> `);
    el.addEventListener('ld-change', () => expect.fail('ld-change incorrectly emitted'));
    el.addEventListener('ld-input', () => expect.fail('ld-change incorrectly emitted'));
    el.checked = true;
    await el.updateComplete;
    el.checked = false;
    await el.updateComplete;
  ...
  it('should hide the native input with the correct positioning to scroll correctly when contained in an overflow', async () => {
    //
    // See: https://github.com/leonardo-design-system/leonardo/issues/1169
    //
    const el = await fixture<LdSwitch>(html` <ld-switch></ld-switch> `);
  ...
  describe('when submitting a form', () => {
    it('should submit the correct value when a value is provided', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <ld-switch name="a" value="1" checked></ld-switch>
    ...
    it('should submit "on" when no value is provided', async () => {
      const form = await fixture<HTMLFormElement>(html`
        <form>
          <ld-switch name="a" checked></ld-switch>
          <ld-button type="submit">Submit</ld-button>
        </form>
      `);
      const button = form.querySelector('ld-button')!;
    ...
    it('should show a constraint validation error when setCustomValidity() is called', async () => {
      ...
      `);
      const button = form.querySelector('ld-button')!;
      const slSwitch = form.querySelector('ld-switch')!;
    ...
    it('should be invalid when required and unchecked', async () => {
      const slSwitch = await fixture<HTMLFormElement>(html` <ld-switch required></ld-switch> `);
      expect(slSwitch.checkValidity()).to.be.false;
    });

    it('should be valid when required and checked', async () => {
      const slSwitch = await fixture<HTMLFormElement>(html` <ld-switch required checked></ld-switch> `);
    ...
    it('should be present in form data when using the form attribute and located outside of a <form>', async () => {
      const el = await fixture<HTMLFormElement>(html`
        <div>
          ...
          <ld-switch form="f" name="a" value="1" checked></ld-switch>
        </div>
      `);
    ...
    it('should receive validation attributes ("states") even when novalidate is used on the parent form', async () => {
      const el = await fixture<HTMLFormElement>(html` <form novalidate><ld-switch required></ld-switch></form> `);
      const slSwitch = el.querySelector<LdSwitch>('ld-switch')!;
  ...
  describe('when resetting a form', () => {
    it('should reset the element to its initial value', async () => {
      ...
      `);
      const button = form.querySelector('ld-button')!;
      const switchEl = form.querySelector('ld-switch')!;
  ...
  it('should not jump the page to the bottom when focusing a switch at the bottom of an element with overflow: auto;', async () => {
    // https://github.com/leonardo-design-system/leonardo/issues/1169
    ...
    const switches = el.querySelectorAll<LdSwitch>('ld-switch');
  ...
  runFormControlBaseTests('ld-switch');
});
