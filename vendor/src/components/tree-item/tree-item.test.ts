...
import type LdTreeItem from './tree-item';

describe('<ld-tree-item>', () => {
  let leafItem: LdTreeItem;
  let parentItem: LdTreeItem;

  beforeEach(async () => {
    leafItem = await fixture(html` <ld-tree-item>Node 1</ld-tree-item> `);
  ...
  describe('when the user clicks the expand button', () => {
    describe('and the item is collapsed', () => {
      it('should emit ld-expand and ld-after-expand events', async () => {
        // Arrange
        const expandSpy = sinon.spy();
        const afterExpandSpy = sinon.spy();

        parentItem.addEventListener('ld-expand', expandSpy);
        parentItem.addEventListener('ld-after-expand', afterExpandSpy);
    ...
    describe('and the item is expanded', () => {
      it('should emit ld-collapse and ld-after-collapse events', async () => {
        // Arrange
        const collapseSpy = sinon.spy();
        const afterCollapseSpy = sinon.spy();

        parentItem.addEventListener('ld-collapse', collapseSpy);
        parentItem.addEventListener('ld-after-collapse', afterCollapseSpy);

        parentItem.expanded = true;
        await oneEvent(parentItem, 'ld-after-expand');
  ...
  describe('when the item is lazy', () => {
    it('should emit ld-lazy-change when the lazy attribute is added and removed', async () => {
      // Arrange
      const lazyChangeSpy = sinon.spy();

      parentItem.addEventListener('ld-lazy-change', lazyChangeSpy);
