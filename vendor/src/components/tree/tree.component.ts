...
import LdTreeItem from '../tree-item/tree-item.component.js';
import styles from './tree.styles.js';
import type { CSSResultGroup } from 'lit';

function syncCheckboxes(changedTreeItem: LdTreeItem, initialSync = false) {
  function syncParentItem(treeItem: LdTreeItem) {
  ...
  function syncAncestors(treeItem: LdTreeItem) {
    const parentItem: LdTreeItem | null = treeItem.parentElement as LdTreeItem;

    if (LdTreeItem.isTreeItem(parentItem)) {
      syncParentItem(parentItem);
      syncAncestors(parentItem);
    }
  }

  function syncDescendants(treeItem: LdTreeItem) {
 ...
 * @documentation https://leonardo.style/components/tree
 * @status stable
 * @since 2.0
 *
 * @event {{ selection: LdTreeItem[] }} ld-selection-change - Emitted when a tree item is selected or deselected.
 *
 * @slot - The default slot.
 * @slot expand-icon - The icon to show when the tree item is expanded. Works best with `<ld-icon>`.
 * @slot collapse-icon - The icon to show when the tree item is collapsed. Works best with `<ld-icon>`.
 *
 * @csspart base - The component's base wrapper.
 *
 * @cssproperty [--indent-size=var(--ld-spacing-medium)] - The size of the indentation for nested items.
 * @cssproperty [--indent-guide-color=var(--ld-color-neutral-200)] - The color of the indentation line.
 * @cssproperty [--indent-guide-offset=0] - The amount of vertical spacing to leave between the top and bottom of the
 *  indentation line's starting position.
 * @cssproperty [--indent-guide-style=solid] - The style of the indentation line, e.g. solid, dotted, dashed.
 * @cssproperty [--indent-guide-width=0] - The width of the indentation line.
 */
export default class LdTree extends LeonardoElement {
  ...
  //
  private lastFocusedItem: LdTreeItem | null;
  private readonly localize = new LocalizeController(this);
  private mutationObserver: MutationObserver;
  private clickTarget: LdTreeItem | null = null;

  constructor() {
    super();
    this.addEventListener('focusin', this.handleFocusIn);
    this.addEventListener('focusout', this.handleFocusOut);
    this.addEventListener('ld-lazy-change', this.handleSlotChange);
  }
  ...
  private initTreeItem = (item: LdTreeItem) => {
  ...
  private handleTreeChanged = (mutations: MutationRecord[]) => {
    for (const mutation of mutations) {
      const addedNodes: LdTreeItem[] = [...mutation.addedNodes].filter(LdTreeItem.isTreeItem) as LdTreeItem[];
      const removedNodes = [...mutation.removedNodes].filter(LdTreeItem.isTreeItem) as LdTreeItem[];
  ...
  private syncTreeItems(selectedItem: LdTreeItem) {
  ...
  private selectItem(selectedItem: LdTreeItem) {
    ...
    if (
      ...
      Promise.all(nextSelection.map(el => el.updateComplete)).then(() => {
        this.emit('ld-selection-change', { detail: { selection: nextSelection } });
      });
    }
  }

  private getAllTreeItems() {
    return [...this.querySelectorAll<LdTreeItem>('ld-tree-item')];
  }

  private focusItem(item?: LdTreeItem | null) {
    item?.focus();
  }
  ...
  private handleKeyDown(event: KeyboardEvent) {
    ...
    if (items.length > 0) {
      ...
      const activeItem: LdTreeItem | undefined = items[activeItemIndex];
  ...
  private handleClick(event: Event) {
    const target = event.target as LdTreeItem;
    const treeItem = target.closest('ld-tree-item')!;
    ...
    //
    // See https://github.com/leonardo-design-system/leonardo/issues/1082
    //
  ...
  handleMouseDown(event: MouseEvent) {
    ...
    this.clickTarget = event.target as LdTreeItem;
  }
  ...
  private handleFocusIn = (event: FocusEvent) => {
    ...
    if (LdTreeItem.isTreeItem(target) && !target.disabled) {
  ...
  async handleSelectionChange() {
    ...
    if (isSelectionMultiple) {
      ...
      [...this.querySelectorAll(':scope > ld-tree-item')].forEach((treeItem: LdTreeItem) =>
  ...
  get selectedItems(): LdTreeItem[] {
    const items = this.getAllTreeItems();
    const isSelected = (item: LdTreeItem) => item.selected;
  ...
  getFocusableItems() {
    ...
    return items.filter(item => {
      ...
      const parent: LdTreeItem | null | undefined = item.parentElement?.closest('[role=treeitem]');
