...
import type LdTree from './tree.component.js';
import type LdTreeItem from '../tree-item/tree-item.js';
...
describe('<ld-tree>', () => {
  ...
  beforeEach(async () => {
    el = await fixture(html`
      <ld-tree>
        <ld-tree-item>Node 1</ld-tree-item>
        <ld-tree-item>Node 2</ld-tree-item>
        <ld-tree-item id="expandable">
          Parent Node
          <ld-tree-item>Child Node 1</ld-tree-item>
          <ld-tree-item>
            Child Node 2
            <ld-tree-item>Child Node 2 - 1</ld-tree-item>
            <ld-tree-item>Child Node 2 - 2</ld-tree-item>
          </ld-tree-item>
        </ld-tree-item>
        <ld-tree-item>Node 3</ld-tree-item>
      </ld-tree>
    `);
  });
  ...
  it('should not focus collapsed nodes', async () => {
    // Arrange
    const parentNode = el.children[2] as LdTreeItem;
    const childNode = parentNode.children[1] as LdTreeItem;
  ...
  describe('when a custom expanded/collapsed icon is provided', () => {
    ...
    it('should append a clone of the icon in the proper slot of the tree item', async () => {
      ...
      // Act
      const treeItems = [...el.querySelectorAll('ld-tree-item')];
  ...
  describe('Keyboard navigation', () => {
    ...
    describe('when Home is pressed', () => {
      it('should move the focus to the first tree item in the tree', async () => {
        // Arrange
        const parentNode = el.children[3] as LdTreeItem;
        parentNode.focus();
        await el.updateComplete;
    ...
    describe('when End is pressed', () => {
      it('should move the focus to the last tree item in the tree', async () => {
        // Arrange
        const parentNode = el.children[0] as LdTreeItem;
        parentNode.focus();
        await el.updateComplete;
    ...
    describe('when Enter is pressed', () => {
      describe('and selection is "single"', () => {
        it('should select only one tree item', async () => {
          // Arrange
          el.selection = 'single';
          const node = el.children[1] as LdTreeItem;
          node.focus();
          await el.updateComplete;
      ...
      describe('and selection is "leaf"', () => {
        it('should select only one tree item', async () => {
          // Arrange
          el.selection = 'leaf';
          const node = el.children[0] as LdTreeItem;
          node.focus();
          await el.updateComplete;
  ...
  describe('Interactions', () => {
    ...
    describe('when the user clicks the expand button', () => {
      it('should expand the tree item', async () => {
        ...
        const node = el.children[2] as LdTreeItem;
    ...
    describe('when the user clicks on a tree item', () => {
      describe('and selection is "single"', () => {
        it('should select only one tree item', async () => {
          // Arrange
          el.selection = 'single';
          const node0 = el.children[0] as LdTreeItem;
          const node1 = el.children[1] as LdTreeItem;
      ...
      describe('and selection is "multiple"', () => {
        ...
        it('should set the indeterminate state to tree items if a child is selected', async () => {
          ...
          const parentNode = el.children[2] as LdTreeItem;
          const childNode = parentNode.children[0] as LdTreeItem;
    ...
    describe('when selection is "single"', () => {
      describe('and user clicks on same item twice', () => {
        it('should emit `ld-selection-change` event once', async () => {
          // Arrange
          el.selection = 'single';
          await el.updateComplete;

          const selectedChangeSpy = sinon.spy();
          el.addEventListener('ld-selection-change', selectedChangeSpy);
  ...
  describe('when selection is "leaf"', () => {
    ...
    describe('and user clicks on expandable item', () => {
      it('should not emit `ld-selection-change` event', async () => {
        // Arrange
        el.selection = 'leaf';
        await el.updateComplete;
        ...
        const node = el.querySelector<LdTreeItem>('#expandable')!;
  ...
  describe('when selection is "multiple"', () => {
    describe('and user clicks on same item twice', () => {
      it('should emit `ld-selection-change` event twice', async () => {
        // Arrange
        el.selection = 'multiple';
        await el.updateComplete;
    ...
    describe('when the tree gets initialized', () => {
      describe('and a parent node is selected', () => {
        it('should select all the nested children', async () => {
          // Arrange
          const tree = await fixture<LdTree>(html`
            <ld-tree selection="multiple">
              <ld-tree-item selected>
                Parent Node
                <ld-tree-item selected>Child Node 1</ld-tree-item>
                <ld-tree-item>
                  Child Node 2
                  <ld-tree-item>Child Node 2 - 1</ld-tree-item>
                  <ld-tree-item>Child Node 2 - 2</ld-tree-item>
          ...
          `);
          const treeItems = Array.from<LdTreeItem>(tree.querySelectorAll('ld-tree-item'));
