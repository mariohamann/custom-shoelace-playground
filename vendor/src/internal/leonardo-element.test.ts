...
import LdButton from '../../dist/components/button/button.component.js';

// We don't use LeonardoElement directly because it shouldn't exist in the final bundle.
/* eslint-disable */
const LeonardoElement = Object.getPrototypeOf(LdButton);
/* eslint-enable */
...
it('Should provide a console warning if attempting to register the same tag twice', () => {
  class MyButton extends LdButton {
    static version = '0.4.5';
  }

  const stub = Sinon.stub(console, 'warn');

  expect(Boolean(window.customElements.get('ld-button'))).to.be.false;
  /* eslint-disable */
  LdButton.define('ld-button');
  expect(Boolean(window.customElements.get('ld-button'))).to.be.true;
  MyButton.define('ld-button');
  /* eslint-enable */
  ...
  expect(warning).to.match(
    new RegExp(
      /* eslint-disable */
      `Attempted to register <ld-button> v${MyButton.version}, but <ld-button> v${LdButton.version} has already been registered`
      /* eslint-enable */
    ),
    'i'
  );
...
it('Should register dependencies when the element is constructed the first time', () => {
  /* eslint-disable */
  class MyElement extends LeonardoElement {
    static dependencies = { 'ld-button': LdButton };
  ...
  new (window.customElements.get('ld-element'))();
  /* eslint-enable */
