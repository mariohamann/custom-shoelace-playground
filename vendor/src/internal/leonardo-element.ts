...
// event requires it)
type LdEventInit<T> = T extends keyof GlobalEventHandlersEventMap
  ? GlobalEventHandlersEventMap[T] extends CustomEvent<Record<PropertyKey, unknown>>
    ? GlobalEventHandlersEventMap[T] extends CustomEvent<Record<PropertyKey, never>>
...
export default class LeonardoElement extends LitElement {
  ...
  static dependencies: Record<string, typeof LeonardoElement> = {};

  constructor() {
    super();
    Object.entries((this.constructor as typeof LeonardoElement).dependencies).forEach(([name, component]) => {
      (this.constructor as typeof LeonardoElement).define(name, component);
    });
  }
}

export interface LeonardoFormControl extends LeonardoElement {
  // Form attributes
  name: string;
  value: unknown;
  disabled?: boolean;
