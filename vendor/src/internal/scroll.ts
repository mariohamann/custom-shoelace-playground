...
export function lockBodyScrolling(lockingEl: HTMLElement) {
  ...
  if (!document.body.classList.contains('ld-scroll-lock')) {
    const scrollbarWidth = getScrollbarWidth(); // must be measured before the `ld-scroll-lock` class is applied
    document.body.classList.add('ld-scroll-lock');
    document.body.style.setProperty('--ld-scroll-lock-size', `${scrollbarWidth}px`);
  }
}
...
export function unlockBodyScrolling(lockingEl: HTMLElement) {
  ...
  if (locks.size === 0) {
    ...
    document.body.style.removeProperty('--ld-scroll-lock-size');
  }
}
