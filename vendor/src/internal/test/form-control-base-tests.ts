...
import type { LeonardoFormControl } from '../leonardo-element.js';

type CreateControlFn = () => Promise<LeonardoFormControl>;

/** Runs a set of generic tests for Leonardo form controls */
export function runFormControlBaseTests<T extends LeonardoFormControl = LeonardoFormControl>(
  tagNameOrConfig:
    | string
    | {
        tagName: string;
...
//
// Applicable for all Leonardo form controls. This function checks the behavior of:
//   - `.validity`
//   - `.validationMessage`,
//   - `.checkValidity()`
//   - `.reportValidity()`
...
function runAllValidityTests(
  tagName: string, //
  displayName: string,
  createControl: () => Promise<LeonardoFormControl>
) {
  ...
  describe(`Form validity base test for ${displayName}`, async () => {
    ...
    it('should not emit an `ld-invalid` event when `.checkValidity()` is called while valid', async () => {
      const control = await createControl();
      const emittedEvents = checkEventEmissions(control, 'ld-invalid', () => control.checkValidity());
      expect(emittedEvents.length).to.equal(0);
    });

    it('should not emit an `ld-invalid` event when `.reportValidity()` is called while valid', async () => {
      const control = await createControl();
      const emittedEvents = checkEventEmissions(control, 'ld-invalid', () => control.reportValidity());
      expect(emittedEvents.length).to.equal(0);
    });

    // TODO: As soon as `LdRadioGroup` has a property `disabled` this
    // condition can be removed
    if (tagName !== 'ld-radio-group') {
      it('should not emit an `ld-invalid` event when `.checkValidity()` is called in custom error case while disabled', async () => {
        const control = await createControl();
      ...
      it('should not emit an `ld-invalid` event when `.reportValidity()` is called in custom error case while disabled', async () => {
        const control = await createControl();
        control.setCustomValidity('error');
        control.disabled = true;
        await control.updateComplete;
...
//
//  Special tests for <ld-button type="button">
//
function runSpecialTests_slButtonOfTypeButton(createControl: CreateControlFn) {
  ...
  it('should not emit an `ld-invalid` event when `.checkValidity()` is called in custom error case, and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
  ...
  it('should not emit an `ld-invalid` event when `.reportValidity()` is called in custom error case, and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    control.disabled = false;
    await control.updateComplete;
...
//
// Special tests for <ld-button href="...">
//
function runSpecialTests_slButtonWithHref(createControl: CreateControlFn) {
  ...
  it('should not emit an `ld-invalid` event when `.checkValidity()` is called in custom error case', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    await control.updateComplete;
    const emittedEvents = checkEventEmissions(control, 'ld-invalid', () => control.checkValidity());
  ...
  it('should not emit an `ld-invalid` event when `.reportValidity()` is called in custom error case', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    await control.updateComplete;
    const emittedEvents = checkEventEmissions(control, 'ld-invalid', () => control.reportValidity());
...
function runSpecialTests_standard(createControl: CreateControlFn) {
  ...
  it('should emit an `ld-invalid` event when `.checkValidity()` is called in custom error case and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    control.disabled = false;
    await control.updateComplete;
  ...
  it('should emit an `ld-invalid` event when `.reportValidity()` is called in custom error case and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    control.disabled = false;
    await control.updateComplete;
...
// Creates a testable Leonardo form control instance
async function createFormControl<T extends LeonardoFormControl = LeonardoFormControl>(tagName: string): Promise<T> {
...
function checkEventEmissions(control: LeonardoFormControl, eventType: string, action: () => void): Event[] {
...
// Component `ld-button` behaves quite different to the other components. To keep things simple we use simple conditions
// here. `ld-button` might stay the only component in Leonardo core behaves that way, so we just hard code it here.
function getMode(control: LeonardoFormControl) {
  if (
    control.localName === 'ld-button' && //
    'href' in control &&
    'type' in control &&
    control.type === 'button' &&
    !control.href
  ...
  // <ld-button href="...">
  if (control.localName === 'ld-button' && 'href' in control && !!control.href) {
