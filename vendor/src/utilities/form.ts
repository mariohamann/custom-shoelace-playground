...
/**
 * Returns all form controls that are associated with the specified form. Includes both native and Leonardo form
 * controls. Use this function in lieu of the `HTMLFormElement.elements` property, which doesn't recognize Leonardo
 * form controls.
 */
export function getFormControls(form: HTMLFormElement) {
  ...
  const leonardoFormControls = collection ? Array.from(collection) : [];

  // To return form controls in the right order, we sort by DOM index
  return [...formControls, ...leonardoFormControls].sort((a: Element, b: Element) => {
